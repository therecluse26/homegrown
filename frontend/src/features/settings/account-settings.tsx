...
// Password management is delegated to Hearth's hosted account settings. [ARCH ADR-020]
const HEARTH_ACCOUNT_URL =
  (import.meta.env.VITE_HEARTH_URL as string | undefined) ?? "http://localhost:4933";
...
export function AccountSettings() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {/* Password — managed via Hearth's hosted account settings [ARCH ADR-020] */}
      <Card className="mb-4">
        <div className="flex items-center justify-between">
          ...
          <Button
            variant="secondary"
            size="sm"
            onClick={() => window.open(HEARTH_ACCOUNT_URL + "/account", "_blank", "noopener")}
          >
            <Icon icon={Key} size="xs" aria-hidden className="mr-1.5" />
            <FormattedMessage id="settings.account.password.change" />
            <Icon icon={ExternalLink} size="xs" aria-hidden className="ml-1.5" />
          </Button>
        </div>
      </Card>
