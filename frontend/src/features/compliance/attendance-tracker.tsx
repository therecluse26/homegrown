        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <button
              ...
              className={`aspect-square rounded-sm flex items-center justify-center type-label-sm relative transition-colors touch-target ${
                entry
            ...
            >
              {day}
              {entry?.is_auto && (
                <span className="absolute bottom-0.5 right-0.5 w-1.5 h-1.5 rounded-full bg-tertiary" />
              )}
            </button>
          );
        })}
      ...
      <Card className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 pt-3 border-t border-outline-variant/20">
          ...
          {(Object.keys(STATUS_COLORS) as AttendanceStatus[]).map(
            (status) => (
              <div key={status} className="flex items-center gap-1">
                <span
                  className={`w-3 h-3 rounded-sm ${STATUS_COLORS[status]}`}
                />
          ...
          <div className="flex items-center gap-1">
            <span className="relative w-3 h-3 rounded-sm bg-surface-container-high">
              <span className="absolute bottom-0 right-0 w-1.5 h-1.5 rounded-full bg-tertiary" />
            </span>
            <span className="type-label-sm text-on-surface-variant">
